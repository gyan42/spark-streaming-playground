...
class TimesofindiaSpider(CrawlSpider):
    ...
    def parse_article(self, response): 
        ...
        # print(response.css('div._38kVl div._3JRp7 div._3Mkg-.byline::text').extract())
        # print(response.css('div._1IaAp.clearfix div._3WlLe.clearfix::text').extract())
        ...
        il.add_css('authors', 'div._38kVl div._3JRp7 div._3Mkg-.byline::text',
                  MapCompose(lambda x:x.split('|', 1)[0] if len(x.split('|', 1)) > 1 else ''))
        il.add_css('published_date', 'div._38kVl div._3JRp7 div._3Mkg-.byline::text',
                   MapCompose(lambda x:x.split('|', 1)[1] if len(x.split('|', 1)) > 1 else ''))
